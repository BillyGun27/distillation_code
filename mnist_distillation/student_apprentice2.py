...
Y_train_new = np.concatenate([Y_train, train_logits_T], axis=1)
Y_test_new =  np.concatenate([Y_test, test_logits_T], axis =1)
...
def knowledge_distillation_loss(y_true, y_pred, alpha,beta,gamma):
    ...
    y_teacher = K.softmax( y_logits )
    
    #teacher , student , apprentice
    loss = ( alpha*logloss(y_true,y_teacher) ) + ( beta*logloss(y_true, y_pred) ) + ( gamma*logloss(y_logits, y_pred) )
...
log_dir = 'logs/loss_apprentice_2/'
